		...
		if (keypress == 0) { // if arrow keys pressed (placeholder for button click)
			...
			while (true) {
				...
				ZoomIn: //zoom in jump placeholder when switching between zooming in and out
					...
					resize(frame1, resize1, Size(frame1.cols * count, frame1.rows * count)); //resizing of the window
	...
	while (true) {
		...
		if (keypress == 0) { // if arrow keys pressed (placeholder for button click)
			...
			while (true) {
				...
				ZoomIn: //zoom in jump placeholder when switching between zooming in and out
					...
					resize(frame2, resize2, Size(frame1.cols * count, frame1.rows * count)); //resizing of the window
		...
		if (keypress == 32) { //if spacebar(32) pressed it zooms out window. Currently placeholder for button click.
			...
			while (true) {
				...
				ZoomOut: //jump area for zooming out from other window/loop
					capture2.read(frame2);
					resize(frame2, resize2, Size(frame1.cols * count, frame1.rows * count)); //resizing frame for zooming out winbdow
					imshow("Zooming Out", resize2);
				}
				else {
					break;
				}
